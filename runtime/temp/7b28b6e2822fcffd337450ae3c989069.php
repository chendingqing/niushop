...
<div class="set-style">
	<dl>
		...
		<dd>
			<input id="brand_name" type="text" placeholder="请输入品牌名称" class="input-common" onchange = "getname()"/>
			<span class="error">请输入品牌名称</span>
			<span class="error brand">已存在品牌名称</span>
		</dd>
	</dl>
	<dl>
...
function addGoodsBrand() {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('ADMIN_MAIN/goods/addgoodsbrand'); ?>",
		data : {
		...
		success : function(data) {
			if (data["code"] > 0) {
				...
				location.href = "<?php echo __URL('ADMIN_MAIN/goods/goodsbrandlist'); ?>";
			}else{
...
function changeCategory(type) {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('ADMIN_MAIN/goods/changecategory'); ?>",
		data : { 'pid' : pid },
		success : function(data) {
			if (type == 1) {
...
function getname(){
	...
	$.ajax({
		url : "<?php echo __URL('ADMIN_MAIN/goods/brandname'); ?>",
		type : "post",
		data : {
			"brand_name" : brand_name
		},
